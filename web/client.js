...
function loadAsyncComponents (to, from, next) {
  ...
  try {
    ...
    this.$app.$emit('routeChanged', to, from, err)
    next()
  }
}
...
async function render (to, from, next) {
  ...
  try {
    ...
    if (error.message === 'ERR_REDIRECT') {
      return this.$app.$emit('routeChanged', to, from, error)
    }
    _lastPaths = []
    ...
    this.error(error)
    this.$app.$emit('routeChanged', to, from, error)
    next()
  }
}
...
function nuxtReady (_app) {
  window.onAppReadyCbs.forEach((cb) => {
    if (typeof cb === 'function') {
      cb(_app)
    }
  })
  // Special JSDOM
  if (typeof window._onAppLoaded === 'function') {
    window._onAppLoaded(_app)
  }
  // Add router hooks
  router.afterEach((to, from) => {
    // Wait for fixPrepatch + $data updates
    Vue.nextTick(() => _app.$app.$emit('routeChanged', to, from))
  })
}
...
function hotReloadAPI(_app) {
  ...
  let $components = getNuxtChildComponents(_app.$app, [])
